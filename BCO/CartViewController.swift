...
//        let dateObj = dateFormatter.date(from: pickupTime.text!)
        ...
        if(pickupTime.text!.isEmpty || quantityTF.text!.isEmpty || Int(quantityTF.text!)! <= 0 )
        {
            self.displayMyAlertMessage(userMessage: "All fields are required in the given range")
        }
        else
        {
    ...
    @IBAction func Cartunwind(unwindSegue:UIStoryboardSegue)
    {
        ...
        let a = unwindSegue.source as! PaymentViewController
